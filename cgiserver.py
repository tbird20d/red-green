...
NOTE: to debug this, add, or uncomment the self.log_message() statements
These message will appear on the console where the server was started.
...
class cgiServerRequestHandler(CGIHTTPServer.CGIHTTPRequestHandler):
    ...
    def is_cgi(self):
        ...
        Return True or False if self.path requires running a
        CGI script, None if not.
        ...
        I added support for checking for '.py' and '.cgi' extensions.
	...
	log_path = re.sub("%","%%", full_url)
        ...
        # also check if path elementi ends in .cgi or .py
...
def test(HandlerClass = cgiServerRequestHandler,
